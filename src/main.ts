...
type JobType = "summarize" | "explain" | "speech-to-text" | "code-review";

type OutputType = string | void;
...
async function processJobEvent(event: NDKEvent, type: JobType): Promise<void> {
    ...
    const waitForPaymentViaZap = (payReqEvent, resolve, reject): void => {
    ...
    const waitForPaymentViaLNInvoice = (payReqEvent, resolve): void => {
